...
def video_process(config: dict, detect_model: detectModel, segment_model: segmentModel, register_model: registerModel):
    ...
    for video in video_list:
        ...
        # detect
        # video_imgs_path = os.path.join(video_path, video)
        ...
        #         pred = segment_model.forward(image)
        #         if not os.path.exists(mask125_save_dir):
        #             os.makedirs(mask125_save_dir)
        #         save_output_mask_rect(imgs[i], pred, mask125_save_dir)

        # register
        # pkl_dir = os.path.join(config['video_path'])
        ...
        # if 1:
        ref_img_path = r'/home/liusheng/data/NFS120/cow/vedio/15组2021.5/DVR_Examiner_Export_2021-05-18 142552_Job_0002/2021-05-12/Native Video Files (MP4)/ref10_name'

        save_infer_feature(ref_img_path, os.path.join(ref_img_path, 'feature.pkl'), register_model, config)
        exit()
        # f = open(pkl_path, 'rb')
