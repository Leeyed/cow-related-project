...
from modules_utils.tools import normPRED
import cv2
import numpy as np
import math
import copy
...
def mask_img(cv2_img, predict):
    ...
    # resize mask
    mask_np = cv2.resize(bkg, (cv2_img.shape[1], cv2_img.shape[0]), interpolation=cv2.INTER_NEAREST)
    try:
    ...
    pcd_img = mask_np_3 * cv2_img + mask_np_3_125
...
class segmentModel:
    def __init__(self, config):
        ...
        if torch.cuda.is_available():
            self.net.load_state_dict(torch.load(config['seg_model_path']))
            self.net.cuda()
        else:
            self.net.load_state_dict(torch.load(config['seg_model_path'], map_location='cpu'))
        self.net.eval()

    def forward(self, img):
        # cv2 img -> skImage
        input_img = img_as_float(img)
        input_img = transform.resize(input_img, (320, 320), mode='constant')
        input_img = input_img.transpose(2, 0, 1)
        input_img = torch.FloatTensor(input_img)
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        input_img = input_img.unsqueeze(0).to(device)
        d1, d2, d3, d4, d5, d6, d7 = self.net.forward(input_img)
        # normalization
        pred = d1[:, 0, :, :]
        pred = normPRED(pred)
        del d1, d2, d3, d4, d5, d6, d7
        return mask_img(img, pred)
