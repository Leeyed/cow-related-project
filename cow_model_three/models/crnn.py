...
class CRNN_new(nn.Module):
    ...
    def __init__(self, class_num, num):
        ...
        self.fc_classify = nn.Linear(d_model, self.class_num)
        # 加个分类对号码牌的数字加以限制
        self.fc_classify_num = nn.Linear(d_model, num)
    ...
    def forward(self, input):
        ...
        x = self.encoder(x)
        outputs = self.fc_classify(x)
        # use class num classfy
        y = F.adaptive_avg_pool2d(x1, (1, 1)).view(x1.size(0), -1)
        y_out = self.fc_classify_num(y)

        return outputs, cnn_features, y_out
