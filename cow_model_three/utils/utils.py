...
def aspectaware_resize_padding_new(image, width, height, interpolation=None, means=None):
    old_h, old_w, c = image.shape
    if old_w > old_h:
        new_w = width
        new_h = int(width / old_w * old_h)
...
def padding(img, img_size, i, framed_metas):
    # padding
    new_image = np.zeros((img_size, img_size, 3))
    # framed_metas[0]==w
    new_image[0:framed_metas[i][1], 0:framed_metas[i][0], :] = img
    return new_image


# add new preprocess 先resize在normalized
def preprocess_new(*image_path, max_size=512, mean=(0.406, 0.456, 0.485), std=(0.225, 0.224, 0.229)):
    ori_imgs = [cv2.imread(img_path) for img_path in image_path]
    imgs_meta = [aspectaware_resize_padding_new(img[..., ::-1], max_size, max_size,
                                                means=None) for img in ori_imgs]
    framed_imgs = [img_meta[0] for img_meta in imgs_meta]
    framed_metas = [img_meta[1:] for img_meta in imgs_meta]
    # 由于训练的时候,图片是先进行通道转换的,然后在进行normalized的,这里的图片处理也是这样的,所以对应的mean,std也要和训练保持一致
    mean = (0.485, 0.456, 0.406)
    std = (0.229, 0.224, 0.225)
    normalized_imgs = [(img / 255 - mean) / std for img in framed_imgs]
    # padding
    padding_imgs = [padding(img, max_size, i, framed_metas) for i, img in enumerate(normalized_imgs)]

    return ori_imgs, padding_imgs, framed_metas


# video
def preprocess_new_video(*frame_from_video, max_size=512, mean=(0.406, 0.456, 0.485), std=(0.225, 0.224, 0.229)):
    ori_imgs = frame_from_video
    # t1 = time.time()
    imgs_meta = [aspectaware_resize_padding_new(img[..., ::-1], max_size, max_size,
                                                means=None) for img in ori_imgs]
