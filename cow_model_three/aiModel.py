...
class Ai_model_three:
    ...
    #         if s > 0:
    #             img_cut = img_ori[int(y0):int(y1), int(x0):int(x1), :]
    #             ### clear blurry detection
    #             obj, classfy_cb_score = self.classfy_cb_model.forward(img_cut)
    #             if obj == 'clear' and classfy_cb_score > self.classfy_cb_model.threshold:
    #                 obj, classfy_score = self.classfy_model.forward(img_cut)
    #                 if classfy_score > 0.85:
    #                     one_obj_data["label"] = obj
    #                     one_obj_data["box"] = [int(x0), int(y0), int(x1), int(y1), float(classfy_score)]
    #                 else:
    #                     one_obj_data["label"] = 'clear'
    #                     one_obj_data["box"] = [int(x0), int(y0), int(x1), int(y1), float(classfy_cb_score)]
    #             # else:
    #             #     one_obj_data["label"] = 'blurry'
    #             #     one_obj_data["box"] = [float(-1)]
    #                 data.append(one_obj_data)
    #             # elif obj == 'clear':
    #     return data
