...
class_name_path = './data/my_data/data.names'  # 将所有类别拉成一类
all_class_name_path = './data/my_data/data.names'  # 原始数据所有类别集合
# 是否拉成一个类别标志
one_label = True
# Nfs data path
...
batch_size = 4
img_size = [512, 512]  # Images will be resized to `img_size` and fed to the network, size format: [width, height]
...
optimizer_name = 'momentum'  # Chosen from [sgd, momentum, adam, rmsprop] 'momentum'
save_optimizer = True  # Whether to save the optimizer parameters into the checkpoint file.
learning_rate_init = 1e-4
lr_type = 'exponential'  # Chosen from [fixed, exponential, cosine_decay, cosine_decay_restart, piecewise] 'piecewise'
...
# mAP eval
eval_threshold = 0.75  # the iou threshold applied in mAP evaluation
...
if one_label:
    ...
    classes = read_class_names(all_class_name_path)
    class_num = len(classes)
...
score_threshold_list = [0.5, 0.6, 0.7, 0.8, 0.9, 0.99]