...
from model.utils import shuffle_and_overwrite, make_summary, config_learning_rate, config_optimizer, AverageMeter
...
if args.save_optimizer:
    ...
    saver_to_save = tf.train.Saver(max_to_keep=3)  # 最多保存5个模型
    saver_best = tf.train.Saver()
...
with tf.Session() as sess:
    ...
    for epoch in range(args.total_epoches):
        ...
        if epoch % args.save_epoch == 0 and epoch > 0:
            if loss_total.average <= 100.:
                saver_to_save.save(sess, args.save_dir + 'Cow_qcj_0831_model-epoch_{}_step_{}_loss_{:.4f}_lr_{:.5g}_one'.format(epoch, int(__global_step), loss_total.average, __lr))
        ...
        if epoch % args.val_evaluation_epoch == 0 and epoch >= args.warm_up_epoch:
            ...
            # print("val_preds: ", val_preds)  # (img_id, xmin, ymin, xmax, ymax, score, label)
            # 根据不同的阈值来计算Recall, Precison, mAP
            for pre_t in args.score_threshold_list:
                print("当前阈值为: ", pre_t)
                # calc mAP
                rec_total, prec_total, ap_total = AverageMeter(), AverageMeter(), AverageMeter()
                gt_dict = parse_gt_rec(args.val_file, args.img_size, args.letterbox_resize)
                ...
                for ii in range(args.class_num):
                    npos, nd, rec, prec, ap = voc_eval(gt_dict, new_list, ii, iou_thres=args.eval_threshold,
                ...
                npos, nd, rec, prec, ap = voc_eval_for_confindence(gt_dict, new_list, args.class_num,
                                                                   ...
                                                                   use_07_metric=args.use_voc_07_metric)
                # info += 'Condience EVAL: Recall: {:.4f}, Precison: {:.4f}, mAP: {:.4f}\n'.format(rec,
                #                                                                        prec, ap)
            ...
            if mAP > best_mAP:
                best_mAP = mAP
                # print('best_model_Epoch_{}_step_{}_mAP_{:.4f}_loss_{:.4f}_lr_{:.7g}'.format(
