...
def parse_line(line):
    ...
    old line format: line_index, img_path, img_width, img_height, [box_info_1 (5 number)], ...
    line format: line_index, img_path, [box_info_1 (5 number)], ...
    return:
        line_idx: int64
        pic_path: string.
    ...
    # 将类别名转化为0,1,2.....
    classes_list = []
    for line in open(args.all_class_name_path).readlines():
        classes_list.append(line.split("\n")[0])
    #
    for i in range(box_cnt):
        label, x_min, y_min, x_max, y_max = None, None, None, None, None
        # 判断是否是拉成一个类别
        if args.one_label:
            ...
            # 当是体况评分时,将号码牌拉成一类
            if s[i*5].startswith('num'):
                s[i * 5] = 'num'
            label, x_min, y_min, x_max, y_max = classes_list.index(s[i * 5]), float(s[i * 5 + 1]), float(s[i * 5 + 2]), float(
                s[i * 5 + 3]), float(s[i * 5 + 4])
...
def process_box(boxes, labels, img_size, class_num, anchors):
    ...
    if boxes.shape[0] > 0:
        ...
        for i, idx in enumerate(best_match_idx):
            ...
            # 每个GT唯一对应最优的feature map
            y_true[feature_map_group][y, x, k, :2] = box_centers[i]
            y_true[feature_map_group][y, x, k, 2:4] = box_sizes[i]
            y_true[feature_map_group][y, x, k, 4] = 1.
            y_true[feature_map_group][y, x, k, 5 + c] = 1.
            y_true[feature_map_group][y, x, k, -1] = boxes[i, -1]
            # 使每个GT都对应三层feature map,并不是唯一对应
            # list_temp = [32., 16., 8.]
            # for k in range(3):
            #     ratio = list_temp[k]
            #     x = int(np.floor(box_centers[i, 0] / ratio))
...
def parse_data(line, class_num, img_size, anchors, mode, letterbox_resize):
    ...
    if not isinstance(line, list):
        ...
        # 加上nfs路径
        pic_path = os.path.join(args.nfs_path, pic_path)
        img = cv2.imread(pic_path)
        # 保证脖带的原始形状,对图片进行填充
        # if args.project == 'bodai':
        #     ori_h, ori_w, _ = img.shape
        #     if ori_w >= ori_h:
        #         tmp = ori_w - ori_h
        #         img = cv2.copyMakeBorder(img, 0, tmp, 0, 0, cv2.BORDER_CONSTANT, value=[0, 0, 0])
        #     else:
        #         tmp = ori_h - ori_w
        #         img = cv2.copyMakeBorder(img, 0, 0, 0, tmp, cv2.BORDER_CONSTANT, value=[0, 0, 0])
        ...
        # 加上nfs路径
        pic_path1 = os.path.join(args.nfs_path, pic_path1)
        img1 = cv2.imread(pic_path1)

        img_idx, pic_path2, boxes2, labels2 = parse_line(line[1])
        # 加上nfs路径
        pic_path2 = os.path.join(args.nfs_path, pic_path2)
...
def get_batch_data(batch_line, class_num, img_size, anchors, mode, multi_scale=False, mix_up=False, letterbox_resize=True, interval=10):
    ...
    if multi_scale and mode == 'train':
        ...
        if img_size[0] < 1024:
            random_img_size = [[x * 32, x * 32] for x in range(10, 20)]  # 320--608
        else:
            random_img_size = [[x * 32, x * 32] for x in range(29, 36)]  # 928--1120
        img_size = random.sample(random_img_size, 1)[0]
        # print("random img size: ", img_size)
    iter_cnt += 1
...
def get_batch_data_project(batch_line, class_num, img_size, anchors, mode, project, multi_scale=False, mix_up=False, letterbox_resize=True, interval=10):
    '''
    generate a batch of imgs and labels
    param:
        batch_line: a batch of lines from train/val.txt files
    ...
    if multi_scale and mode == 'train':
        random.seed(iter_cnt // interval)
        if project == 'COW' or project == 'bodai':
            random_img_size = [[x * 32, x * 32] for x in range(10, 20)]  # 320--608
            img_size = random.sample(random_img_size, 1)[0]
        elif project == 'BCS':
            random_img_size = [[x * 32, x * 32] for x in range(29, 36)]  # 928--1120
            ...
            random_img_size = [[x * 32, x * 32] for x in range(19, 30)]  # 608--928
            img_size = random.sample(random_img_size, 1)[0]
        # print("random img size: ", img_size)
    iter_cnt += 1
