...
class trackModel:
    ...
    def forward(self, img):
        ...
        for i in range(len(boxes_)):
            ...
            features.append(test_feature.detach().numpy())
            # test_feature_matrix = test_feature.expand_as(self.ref_feature_tensor)
            # if torch.cuda.is_available():
            #     predicts = torch.cosine_similarity(test_feature_matrix, self.ref_feature_tensor).cpu().numpy().reshape(-1)
            # else:
            ...
            # cv2.putText(img_paint, 'cow', (x0, y0), 0, 4, (0, 255, 0), 4)
        # return img_paint

        # deep sort
        boxs = self.trans_boxs(boxes_)
        ...
        for i, track in enumerate(self.tracker.tracks):
            ...
            cv2.putText(img_paint, str(track.track_id), (int(bbox[0]), int(bbox[1])), 0, 4, (0, 255, 0), 4)
            # track_flag = True
            # break
            # if people_num < track.track_id:
            #     people_num = track.track_id
        # 目标丢失, 重新识别
        # if not track_flag:
        #     self.start_track = False
        ...
        # 求取IOU最大
        # if len(trackBoxes)>0:
        #     iou = self.calc_iou(trackBoxes, np.array(track_boxes))
        #     iou_list = iou[0].tolist()
        #     max_index = iou_list.index(max(iou_list))
        #
        #     track_box = self.tracker.tracks[max_index].to_tlbr()
        #     self.track_id = self.tracker.tracks[max_index].track_id
        #     cv2.rectangle(img_ori, (int(track_box[0]), int(track_box[1])), (int(track_box[2]), int(track_box[3])),
        #                   (255, 255, 255), 2)
        #
        #     cv2.putText(img_ori, str(self.track_id), (int(track_box[0]), int(track_box[1])), 0, 5e-3 * 200, (0, 255, 0),
        #                 2)
